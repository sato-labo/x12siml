...
#vat=c(1997,1)
	...
	z <- sarsiml(data = data, reg = reg, trend = trend, ilog = 
	...
	if(!is.null(rpp)) {
		tmp <- sarsiml(data = data, reg = reg, trend = trend,
...
#vat=c(1997,1)
	...
	for(i in 1:(k)) {
		switch(type,
			ao = {
				tmp <- sarsiml(data = data, reg = reg, trend
				...
				for(m2 in (m1):(min(m1 + 1, j - 1))) {
				  ...
				  if(m1 == m2)
				    tmp <- sarsiml(data = data, reg = reg, 
				      ...
				      ...)
				  else tmp <- sarsiml(data = data, reg = reg, 
				      trend = trend,  ilog = ilog, 
				      frequency = frequency, start = start, 
		...
		if(rpp[4] == rpp[6])
			tmp <- sarsiml(data = data, reg = reg, trend = trend, 
				...
				start = start, iplot = T, sorder = sorder,mtype=mtype, tcrp
				 = c(rpp[1:4], rpp[7:8]), ...)
		else tmp <- sarsiml(data = data, reg = reg, trend = trend, ar
...
"avg12" <- function(x,...) {
    ...
    zz <- sarsiml(x,trend=m1,sorder=m2,frequency=12,...)
